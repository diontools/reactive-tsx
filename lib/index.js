...
exports.reactiveArray = function (init) {
    ...
    var _actions = [];
    var _listeners = [];
    var _holders = init.map(function (_, i) { return createHolder(i); });
    var length = exports.reactive(_v.length);
    function createHolder(indexValue) {
        var item = exports.reactive(_v[indexValue]);
        var index = exports.reactive(indexValue);
        var unsubscribe = item.subscribe(function () { return _v[index.value] = item.value; }, true);
        return ({ item: item, index: index, unsubscribe: unsubscribe });
    }
