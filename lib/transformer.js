...
function createCallConditional(context, unsubscribesId, exp, parentNodeId, onUpdateId, nestCount) {
    ...
    if (parentNodeId) {
        ...
        onUpdate = ts.createArrowFunction(undefined, undefined, [nodeParameter], undefined, undefined, ts.createAssignment(currentNodeId, ts.createCall(ts.createIdentifier(ReplaceNodeFunctionName), undefined, [nodeId, currentNodeId, parentNodeId])));
    }
...
function createUpdateText(context, onUpdateId, expression) {
    // onUpdate(expression || '')
    return ts.createCall(onUpdateId, undefined, [ts.createLogicalOr(expression, ts.createStringLiteral(''))]);
}
