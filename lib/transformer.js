...
function transformSourceFile(ctx, typeChecker, sourceFile, opts) {
    ...
    const isPrefixAccessModule = module === ts.ModuleKind.CommonJS
        || module === ts.ModuleKind.AMD
        || module === ts.ModuleKind.UMD
        || module === ts.ModuleKind.System;
    const createModuleMemberAccess = (name) => isPrefixAccessModule
...
function transformCombine(context, unsubscribesId, node) {
    ...
    return ts.createCall(context.combineReactiveFuncExp, undefined, [
...
function transformJsxOpeningLike(context, unsubscribesId, jsxNode, parentNodeId, onUpdateId, children) {
    ...
    newStatements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([ts.createVariableDeclaration(elementId, undefined, ts.createCall(context.elementFuncExp, undefined, [tagNameExp]))], ts.NodeFlags.Const)));
    let onCreateEventExpression;
    let onDestroyEventExpression;
    ...
    for (const attribute of jsxNode.attributes.properties) {
        ...
        newStatements.push(createElementPropertyUpdateStatement(context, unsubscribesId, elementId, ts.createStringLiteral(attrName), expression));
    }
    if (children) {
    ...
    if (onCreateEventExpression) {
        // onCreateExp(element)
        newStatements.push(ts.createStatement(ts.createCall(onCreateEventExpression, undefined, [elementId])));
    }
    if (onDestroyEventExpression) {
        // unsubscribes.push(() => onDestroyExp(element))
        newStatements.push(ts.createStatement(ts.createCall(ts.createPropertyAccess(unsubscribesId, 'push'), undefined, [ts.createArrowFunction(undefined, undefined, [], undefined, undefined, ts.createCall(onDestroyEventExpression, undefined, [elementId]))])));
    }
    if (onUpdateId) {
        // onUpdate(elementId)
...
function createElementPropertyUpdateStatement(context, unsubscribesId, elementExp, attrExp, expression) {
    // elementExp[attrExp] = expression
    const assign = ts.createAssignment(ts.createElementAccess(elementExp, attrExp), expression);
    const reactives = getAllReactives(context, expression);
    if (reactives.length === 0) {
        ...
        return ts.createStatement(ts.createCall(ts.createPropertyAccess(unsubscribesId, 'push'), undefined, [ts.createCall(ts.createPropertyAccess(reactives[0], 'subscribe'), undefined, [ts.createArrowFunction(undefined, undefined, [], undefined, undefined, assign)])]));
    }
    else {
        // subscribe$(unsubscribes, [reactives], () => assign)
        return ts.createStatement(ts.createCall(context.subscribeFuncExp, undefined, [
            unsubscribesId,
            ts.createArrayLiteral(reactives),
...
function createStyleStatements(context, unsubscribesId, elementId, expression, statements) {
    if (!ts.isObjectLiteralExpression(expression))
        throw 'style must specify an object literal.';
    const styleAccess = ts.createPropertyAccess(elementId, 'style');
    for (const prop of expression.properties) {
        if (ts.isPropertyAssignment(prop)) {
            const name = ts.isLiteralExpression(prop.name)
                ? ts.createStringLiteral(prop.name.text)
                : ts.isComputedPropertyName(prop.name)
                    ? prop.name.expression
                    : ts.createStringLiteral(ts.idText(prop.name));
            // element.style[name] = initializer
            statements.push(createElementPropertyUpdateStatement(context, unsubscribesId, styleAccess, name, prop.initializer));
        }
        else {
...
function transformClassAttributeExpression(expression) {
    const buffer = [];
...
function transformClassAttributeChildExpression(expression, buffer) {
    if (ts.isStringLiteral(expression)) {
        buffer.push(ts.createStringLiteral(fixClassNameText(expression.text, buffer.length === 0)));
    }
...
function fixClassNameText(text, isFirst) {
    // add space to 2nd or later string
    text = text.trim();
    return isFirst ? text : ' ' + text;
}
function transformClassAttributeChildObjectName(prop, isFirst) {
    let nameExp;
    ...
    // initializer ? nameExp : ''
    return ts.createConditional(prop.initializer, nameExp, ts.createStringLiteral(''));
}
// "foo"
function transformJsxText(context, jsxText, parentNodeId) {
    context.textFuncUsed = true;
    const stringLiteral = createFixedStringLiteral(jsxText.text);
    if (!stringLiteral)
        return undefined; // skip
    // parentNode.appendChild(text$('text'))
    return ts.createCall(ts.createPropertyAccess(parentNodeId, 'appendChild'), undefined, [ts.createCall(context.textFuncExp, undefined, [stringLiteral])]);
}
// {foo}
function transformJsxExpression(context, unsubscribesId, jsxExpression, parentNodeId) {
    ...
    if (ts.isCallExpression(jsxExpression.expression) && ts.isPropertyAccessExpression(jsxExpression.expression.expression)) {
        ...
        if (isAssignable(context.typeChecker, context.reactiveArrayType, objType)) {
            ...
            return ts.createCall(context.mapArrayFuncExp, undefined, [parentNodeId, reactiveArrayExp, createExp]);
        }
    }
    // subscribe reactives
    ...
    return ts.createCall(ts.createPropertyAccess(parentNodeId, 'appendChild'), undefined, [ts.createCall(context.textFuncExp, undefined, [jsxExpression.expression])]);
}
...
function createConditionalStatements(context, unsubscribesId, exp, parentNodeId) {
    ...
    statements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([
        ts.createVariableDeclaration(currentNodeId, undefined, ts.createCall(context.textFuncExp, undefined, [ts.createStringLiteral('')]))
    ], ts.NodeFlags.Let)));
    ...
    const onUpdate = ts.createArrowFunction(undefined, undefined, [nodeParameter], undefined, undefined, ts.createAssignment(currentNodeId, ts.createCall(context.replaceNodeFuncExp, undefined, [nodeId, currentNodeId, parentNodeId])));
...
function createCallConditionalInternal(context, unsubscribesId, exp, parentNodeId, onUpdate, nestCount) {
    ...
    const callConditional = ts.createCall(context.conditionalFuncExp, undefined, [
        unsubscribesId,
...
function createReactiveText(context, unsubscribesId, expression, reactives, parentNodeId) {
    ...
    if (reactives.length === 0) {
        context.textFuncUsed = true;
        // text$(expression || '')
        const textNode = ts.createCall(context.textFuncExp, undefined, [ts.createLogicalOr(expression, ts.createStringLiteral(''))]);
        if (parentNodeId) {
    ...
    statements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([ts.createVariableDeclaration(textNodeId, undefined, ts.createCall(context.textFuncExp, undefined, [ts.createStringLiteral('')]))], ts.NodeFlags.Const)));
    ...
    if (reactives.length === 1) {
        ...
        statements.push(ts.createStatement(ts.createCall(context.subscribeFuncExp, undefined, [unsubscribesId, ts.createArrayLiteral(reactives), actionArrowFunction])));
