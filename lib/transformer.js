...
function isConditionalable(expression) {
    // && binary expression
...
function createCallConditional(context, unsubscribesId, exp, parentNodeId) {
    let conditionExp;
    let trueExp;
    let falseExp;
    ...
    if (!isTrueExp && !isFalseJsx && reactivesInTrue.length === 0 && reactivesInFalse.length === 0) {
        ...
        const statements = [];
        let onUpdate = ts.createIdentifier('undefined');
        if (parentNodeId) {
            ...
            statements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([
                ts.createVariableDeclaration(textNodeId, undefined, ts.createCall(createTextNodeMethod, undefined, [ts.createStringLiteral('')]))
            ], ts.NodeFlags.Const)));
            ...
            onUpdate = ts.createArrowFunction(undefined, undefined, [nodeParameter], undefined, undefined, ts.createAssignment(ts.createPropertyAccess(textNodeId, 'nodeValue'), textId));
        }
        // conditionalText(unsubscribes, [reactives], () => condition, trueExp, falseExp, onUpdate)
        const callConditionalText = ts.createCall(ts.createIdentifier(ConditionalTextFunctionName), undefined, [
            unsubscribesId,
    ...
    if (parentNodeId) {
        ...
        onUpdate = ts.createArrowFunction(undefined, undefined, [nodeParameter], undefined, undefined, ts.createBlock([
            ts.createStatement(ts.createCall(ts.createPropertyAccess(parentNodeId, 'replaceChild'), undefined, [nodeId, currentNodeId])),
...
function statementsToBody(node) {
    if (Array.isArray(node))
        return ts.createBlock(node, true);
    return node;
}
