...
function transformComponent(context, node, defineComponentSymbol) {
    ...
    if (isJex(arrowFunction.body)) {
        ...
        body.statements = ts.createNodeArray(__spreadArrays(body.statements.slice(0, returnIndex), jsxToStatements(transformJsx(context, unsubscribesId, returnStatement.expression, undefined)), body.statements.slice(returnIndex + 1)));
    }
    else {
...
function transformJsxOpeningLike(context, unsubscribesId, jsxNode, parentNodeId, children) {
    ...
    var elementId = ts.createIdentifier(tagNameExp.text + context.nodeNumber++);
    ...
    if (children) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            childStatements.push.apply(childStatements, jsxToStatements(transformJsx(context, unsubscribesId, child, elementId)));
        }
...
function createChildComponentCallExpression(context, unsubscribesId, jsxNode, parentNodeId, children, tagNameExp) {
    ...
    if (children) {
        ...
        for (var i = 0; i < children.length; i++) {
            var child = children[i];
            childStatements.push.apply(childStatements, jsxToStatements(transformJsx(context, unsubscribesId, child, childParentNodeId)));
        }
        if (childStatements.length > 0) {
...
function transformJsxExpression(context, unsubscribesId, jsxExpression, parentNodeId) {
    ...
    if (ts.isCallExpression(jsxExpression.expression) && ts.isPropertyAccessExpression(jsxExpression.expression.expression)) {
        ...
        if (isAssignable(context.typeChecker, context.reactiveArrayType, objType)) {
            ...
            var childUnsubscribesId = ts.createIdentifier('unsubscribes');
            var body = transformJsxToBody(context, childUnsubscribesId, selectExp.body, undefined);
            if (!body)
...
function transformJsxToBody(context, unsubscribesId, node, parentNodeId) {
    var jsx = transformJsx(context, unsubscribesId, node, parentNodeId);
...
function createReactiveText(context, unsubscribesId, expression, reactives, parentNodeId) {
    ...
    var textNodeId = ts.createIdentifier('text' + context.nodeNumber++);
...
function createReactiveTextBlock(context, unsubscribesId, expression, reactives, parentNodeId) {
    var text = createReactiveText(context, unsubscribesId, expression, reactives, parentNodeId);
