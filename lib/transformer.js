...
function transformJsxOpeningLike(context, unsubscribesId, jsxNode, parentNodeId, onUpdateId, children) {
    ...
    newStatements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([ts.createVariableDeclaration(elementId, undefined, ts.createCall(ts.createIdentifier(ElementFunctionName), undefined, [tagNameExp]))], ts.NodeFlags.Const)));
...
function transformJsxText(context, jsxText, parentNodeId) {
    ...
    return ts.createCall(ts.createPropertyAccess(parentNodeId, 'appendChild'), undefined, [ts.createCall(ts.createIdentifier(TextFunctionName), undefined, [stringLiteral])]);
}
// {foo}
function transformJsxExpression(context, unsubscribesId, jsxExpression, parentNodeId) {
    ...
    return ts.createCall(ts.createPropertyAccess(parentNodeId, 'appendChild'), undefined, [ts.createCall(ts.createIdentifier(TextFunctionName), undefined, [jsxExpression.expression])]);
}
...
function createConditionalStatements(context, unsubscribesId, exp, parentNodeId) {
    ...
    statements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([
        ts.createVariableDeclaration(currentNodeId, undefined, ts.createCall(ts.createIdentifier(TextFunctionName), undefined, [ts.createStringLiteral('')]))
    ], ts.NodeFlags.Let)));
...
function createReactiveText(context, unsubscribesId, expression, reactives, parentNodeId) {
    ...
    if (reactives.length === 0) {
        context.textFuncUsed = true;
        // text$(expression || '')
        const textNode = ts.createCall(ts.createIdentifier(TextFunctionName), undefined, [ts.createLogicalOr(expression, ts.createStringLiteral(''))]);
        if (parentNodeId) {
    ...
    statements.push(ts.createVariableStatement(undefined, ts.createVariableDeclarationList([ts.createVariableDeclaration(textNodeId, undefined, ts.createCall(ts.createIdentifier(TextFunctionName), undefined, [ts.createStringLiteral('')]))], ts.NodeFlags.Const)));
